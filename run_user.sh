...
#done
                ...
                #./wrk2/wrk -D exp -t 2 -c 2 -d 30 -s ./wrk2/scripts/social-network/compose-post.lua http://localhost:8080/wrk2-api/post/compose -R $d >> runData
                        ...
                        #sudo python3 ./profiler/profiler.py -n node2 report -d ~/temp/node2/qps$d/repeat$i
                        #sudo python3 ./profiler/profiler.py -n node3 report -d ~/temp/node3/qps$d/repeat$i
...
do
        ...
        do
                ...
                do
                        d=$(($(($c+1))*100))
                        python3 ./profiler/analyze.py ~/temp/node$a/qps$d/repeat$i/ 0 20 30 >> output/cstateAv
                        cat ~/temp/node$a/qps$d/repeat$i/dram >> output/power
                        cat ~/temp/node$a/qps$d/repeat$i/package-0 >> output/power
                        cat ~/temp/node$a/qps$d/repeat$i/package-1 >> output/power
                        echo '-----' >> output/power
                        for (( j=0 ; j<20 ; j++ ));
                        do
                                        l=$(($j+1))
                                        #python3 ./profiler/analyze.py ~/temp/node$a/qps$d/repeat$i/ $j $l 30 >> output/cpuCState
                        done
